...
public class DateUtils {
    ...
    public static ZonedDateTime convertDatetime(String value){
        ...
        try {
            SimpleDateFormat simpleDateFormat = new SimpleDateFormat(PATTERN_DDMMYYYYHHMMSS);
            Date date = simpleDateFormat.parse(value);
    ...
    public static String getDurationByDate(String ending){
        ZonedDateTime currentDay = now();
        ZonedDateTime finishDay = convertDatetime(ending);
        Duration duration = Duration.between(currentDay, finishDay);
        String response = duration.toDays() + "D " + duration.toHours() + "Hrs " + duration.toMinutes() + "min";
        return response;
    }
    public static String getDurationByHours(String ending){
        ZonedDateTime currentDay = now();
        ZonedDateTime finishDay = convertDatetime(ending);
        Duration duration = Duration.between(currentDay, finishDay);
        String response = duration.toHours() + "Hrs " + duration.toMinutes() + "min";
        return response;
    }

    public static String getDurationFinishDayByDate(String currentEnding, String finishDay){
        ZonedDateTime currentFinishDay = convertDatetime(currentEnding);
        ZonedDateTime ending = convertDatetime(finishDay);
        Duration duration = Duration.between(currentFinishDay, ending);
        String response = duration.toDays() + "D " + duration.toHours() + "Hrs " + duration.toMinutes() + "min";
        return response;
    }

    public static String getDurationFinishDayByHours(String currentEnding, String finishDay){
        ZonedDateTime currentFinishDay = convertDatetime(currentEnding);
        ZonedDateTime ending = convertDatetime(finishDay);
        Duration duration = Duration.between(currentFinishDay, ending);
        String response = duration.toHours() + "Hrs " + duration.toMinutes() + "min";
    ...
    public static void main(String[] args) throws ParseException {
        String date = "10/06/2022";
        ZonedDateTime zone = convertShort2Zone(date);
        // System.out.println(format(zone, "dd/MM/yyyy"));
        System.out.println(format(zone, "dd/MM/yyyy"));
    }
