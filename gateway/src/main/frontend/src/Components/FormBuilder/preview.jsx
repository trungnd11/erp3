...
export default class Preview extends React.Component {
  ...
  manualEditModeOff = () => {
    ...
    if (editElement && editElement.dirty) {
      ...
      for (let i = 0, len = data.length; i < len; i++){
        if (editElement.id !== data[i].id && editElement.field_name === data[i].field_name) {
          showErrorNotice('Field name is duplicated!');
          return;
        }
      }
  ...
  _onChange(data) {
    ...
    data.forEach((item) => {
      if (item && item.readOnly && this.props.variables && this.props.variables[item.variableKey]) {
