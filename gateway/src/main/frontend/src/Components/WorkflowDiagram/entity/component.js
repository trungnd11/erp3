...
const EntityContainerHOC = WrappedComponent =>
  ...
  > {
    ...
    constructor(props) {
      ...
      const index = this.props.entities.findIndex(el => el.id === this.props.model.id)
      this.entityData = { ...this.props.entities[index] };
      console.log(props);
    }
    ...
    handleConfigClick = () => {
      const { setOpen } = this.refModal.current;
      setOpen(true);
    }

    render() {
      const { body: Body } = this.props.entityTypes[this.props.model.type];
      return (
        <Entity
        ...
        >
          ...
          <span role="button" tabIndex={0} style={{ position: "absolute", top: 0, left: 0 }} onClick={this.handleClick} >AAA</span>
          <span role="button" className='ri-settings-3-line'
            onClick={this.handleConfigClick}
            style={{ position: "absolute", top: 0, right: 0, fontSize: '1.5rem' }}>
          </span>
          <ConfigModal ref={this.refModal} title={`Setting ${this.props.model.name}`} >
            <Body data={this.props.model} />
          </ConfigModal>
        </Entity>
      );
    }
